...
<c:if test="${not empty cart.cartItems }">
						...
						<c:forEach var="item" items="${cart.cartItems }">
							<tr class="active">
								...
								<td width="10%"><input type="text" name="quantity"
									value="${item.count }" maxlength="4" size="10"></td>
								<td width="15%"><span class="subtotal">￥${item.subTotal}</span>
								</td>
								<td><a href="#" class="delete"
									onclick="deleteByPid('${item.product.pid}')">删除</a></td>
							</tr>
						</c:forEach>
						<script>
							function deleteByPid(pid) {
								//alert(pid);
								var flag = confirm("您真的忍心要删除这个商品吗?");
								if (flag) {
									//跳转去cartServlet
									//alert("确定要删除");
									location.href = "${pageContext.request.contextPath}/cart?method=delete&pid="
											+ pid;
								}
							}
						</script>
		...
		<div style="margin-right: 130px;">
			...
			<div
				...
				<a href="#" onclick="clear1()" id="clear" class="clear">清空购物车</a>
				<script>
					function clear1() {
						var flag = confirm("您真的确定要清空整个购物车吗?");
						if (flag) {
				...
				<a href="${pageContext.request.contextPath }/order?method=submitOrder"> <input type="submit" width="100"
